...
function Main() {
  ...
  const cartsPerGroup = 6; // Số cart trên một nút

  const handleButtonClick = (groupNumber) => {
    setCurrentGroup(groupNumber);
  };

  const currentGroupCart = groups[currentGroup - 1];

  const transformStyle = {
    transform: `translate3d(-${
      (currentGroup - 1) * (100 / cartsPerGroup)
    }%, 0, 0)`,
  };
  ...
  return (
    <div id="main">
      <h3> New For You</h3>
      <div className="main_product_container" style={transformStyle}>
        {currentGroupCart.map((maincart, index) => (
          <MainCart
            key={index}
