...
function Main() {
  ...
  useEffect(() => {
    // Tính toán giá trị của transform dựa trên currentGroup
    const transformValue = `translate3d(-${
      (currentGroup - 1) * 6 * 3289
    }px, 0px, 0px)`;
    // Áp dụng giá trị transform vào main_product để thực hiện chuyển động
    document.querySelector(".main_product").style.transform = transformValue;
  }, [currentGroup]);
