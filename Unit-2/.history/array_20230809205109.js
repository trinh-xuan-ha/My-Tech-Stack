...
/** phương thức sử dụng với DOM classlisst
 * add.  thêm    class
 * contains kt xem có class dó không.
 * remove xóa class.
 * toggle nê có thì xóa nếu không thi thêm
 */
let ha = document.querySelector(".container");
console.log(ha.classList.length);
console.log(ha.classList);
if (ha.classList.contains("check")) {
  ha.classList.remove("check");
}
console.log(ha.classList);
//ha.classList.toggle("color");
// ha.classList.toggle("color");
setTimeout(() => {
  ha.classList.toggle("color");
}, 3000);
...
let haclick = () => {
  ha.classList.toggle("color-o");
};
//!e.target trả về chính nó.

//todo onchange, oninput   e.target.
// DOM events.
//!preventDefault - ngawn chặc mặc định của tronh duyệt, StopPropagation - ngăn chặn trạng thái nổi bọt khi event không ảnh hưởng đến  cha ông của nó..
let checks123 = document.querySelectorAll("a");
for (let i in checks123) {
  if (checks123.hasOwnProperty(i)) {
    checks123[i].onclick = (e) => {
      console.log(e.target.href);
      if (!e.target.href.startsWith("https://www.instagram.com/")) {
        e.preventDefault(); //todo chặn nếu không phải đường dẫn trên thì không cho chuyển trang
        e.stopPropagation();
      }
    };
  }
}
//!event listener. - sủa dụng khi  có nhiều even tường minh
let viec1 = () => {
  alert("ádasdsa");
};
ha.addEventListener("click", viec1);
ha.addEventListener("click", () => {
  console.log("đây là tôi");
});
setTimeout(() => {
  ha.removeEventListener("click", viec1);
}, 3000);
const value = "10";
const multiplier = 2;

const result = value * multiplier; // Gây lỗi ép kiểu không đúng
console.log(result);
const x = 0.1 + 0.2;
const y = 0.3;

if (x === y) {
  console.log("Bằng nhau" + x);
} else {
  console.log("Không bằng nhau" + x); // In ra kết quả này, mặc dù có thể mong muốn kết quả khác
}
