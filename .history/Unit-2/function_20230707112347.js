...
let carts = red(1000,9999.8);
console.log(carts);
//Generator Function: khai báo hàm sử dụng cú pháp hàm generator (`function *`) và từ khóa `yield` để tạo ra một chuỗi giá tị tuần tự.
function* github() {
    yield 1;
    yield 2;
    yield 'Hà trịnh xuân';
...
console.log(lists.next().value);
console.log(lists.next().value);
//Async function: khai báo hàm bất đồng bộ sử dụng từ khóa  `async` ,cho phép sử dụng  `await` để chờ các hàm bất đồng bộ khác hoàn thành.
async function waitData() {
    const data = await fetch('https://www.google.com/');
    const text = await data.j
}  