...
async function load() {
    ...
    console.log(json);
    const listHocSinh = json.user;
    // console.log(listHocSinh);
  //  let check = listHocSinh.length;
    console.log(listHocSinh);
    ...
    for(let i = 0; i < listHocSinh.length; i++) {
        ...
        tr.append(td0);
        td0.innerText = listHocSinh[i].id;

        let td1 = document.createElement('td');
        tr.append(td1);
        td1.innerText = listHocSinh[i].fullName;

        let td2 = document.createElement('td');
        tr.append(td2);
        td2.innerText = listHocSinh[i].date;

        let td3 = document.createElement('td');
        tr.append(td3);
        td3.innerText = listHocSinh[i].address;

        let td4 = document.createElement('td');
        tr.append(td4);
        let sdt = listHocSinh[i].numberphone;
        td4.innerText = '0' + sdt;

        let td5 = document.createElement('td');
        tr.append(td5);
        td5.innerText = listHocSinh[i].point;
