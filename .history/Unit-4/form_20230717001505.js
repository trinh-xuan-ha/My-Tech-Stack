...
function checkDiemError() {
    ...
    let continueButton = document.getElementById('continue_button');
    let diemPattern = /^[1-10]+$/;
    let checkDiem = diemInput.value.trim();
    if(0 > checkDiem || checkDiem > 10 || !diemPattern.test(checkDiem)) {
        diemError.textContent = 'điểm đầu vào lớn hơn 0 và nhở hơn 10.';
        diemError.computedStyleMap.display = 'block';
        continueButton.disabled = true;
        continueButton.classList.remove('form_btn_green')
    }
    else {
